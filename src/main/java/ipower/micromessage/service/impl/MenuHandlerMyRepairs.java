 ...
 * @since 2014-03-10.
 * */
public class MenuHandlerMyRepairs extends MenuArticleHandler {
	...
	protected BaseRespMessage createConent(BaseMessage current,MicroContext context, List<Article> articles) {
		...
		CallbackData callback = this.remoteEICPService.remotePost("MyRepairs", post);
		...
		if(bodys == null || (size = bodys.size()) == 0){
			return this.handlerMessage(current, context, "没有获取到您的报修！");
		}
